...
- **Concurrency curve complete** (log #20): drift-corrected plateau of ~36-38 Gbps across c=2-8 (c=1..256 now all measured); transient 41.2 Gbps on a hot machine matches pure-TCP's 42.8 ceiling. Tool performance is flat across reasonable -c.
- **Machine ceiling saturated** (log #19): pure-TCP 2MB loopback ceiling at c=4 = ~35-37 Gbps (3-run median 35.1); the HTTP pair's peak (36.5) sits exactly on it. The tool has reached the machine's wall — the +53% improvement takes it from 19.4 Gbps to the physical loopback limit.
- **Client is idle at the peak** (log #18): at c=4 the client uses ~1.3 of 12 cores and the pair ~3.7 — not CPU-saturated even at the ceiling. The c=4 limit is HTTP/1.1 request serialization (net/http handoffs + 2 RTTs/request), not client CPU. Optimality now proven from both sides: not CPU-bound at the peak AND not the bottleneck at high concurrency.
